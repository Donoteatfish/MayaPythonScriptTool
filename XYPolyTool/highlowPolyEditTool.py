# -*- coding: utf-8 -*-
#如果你想联系我，这是我的邮箱xiangyu.23@qq.com
...
class highlowPolyEdit():
	...
	def highlowPolyEditUI(self,*args,**kwargs):
		...
		#平滑低模UV----------------------------------------------------------------------
		cmds.columnLayout()
		cmds.text(l=u' 低模按UV边界平滑内部边', h=20, fn ='boldLabelFont')
		cmds.setParent('..')

		cmds.rowColumnLayout(numberOfColumns=1, adj=1)
		cmds.button(l=u'选中并平滑', h=40, c=self.SoftByUVsEdge)
		cmds.setParent('..')
		...
		#重命名-------------------------------------------------------------------------
		cmds.columnLayout()
		cmds.text(l=u' 重命名低模', h=20, fn ='boldLabelFont')
		cmds.setParent('..')

		cmds.rowColumnLayout(numberOfColumns=2, adj=2)
		cmds.text(l=u'名称:', w=55, al='right')
		...
		cmds.text(l=u'起始编号:', w=55, al='right')
		cmds.textField('StartTextField', tx='1', w=90)
		cmds.text(l='')
		cmds.text(l=u'编号位数:', w=55, al='right')
		...
		cmds.rowColumnLayout(numberOfColumns=1, adj=1)
		cmds.button(l=u'重命名低模', h=40, c=self.Rename)
		cmds.setParent('..')
		...
		#设置低模的格式
		cmds.columnLayout()
		cmds.text(l=u' 设置低模格式', h=20, fn ='boldLabelFont')
		cmds.setParent('..')

		cmds.rowColumnLayout(numberOfColumns=5, adj=2)
		cmds.text(l=u'后缀名:', w=55, al='right')
		cmds.textField('LowSubfixTextField', tx='_low')
		cmds.text(l='', w=5)
		cmds.button(l=u'删除', w=45, c=self.DeletLowSuffix)
		cmds.button(l=u'添加', w=45, c=self.AddLowSuffix)
		cmds.setParent('..')

		cmds.separator( height=5, style='none' )

		cmds.rowColumnLayout(numberOfColumns=1, adj=1)
		cmds.button(l=u'选中全部低模并记录', h=40, c=self.RecordLowPoly)
		cmds.setParent('..')
		...
		cmds.columnLayout()
		cmds.text(l=u' 设置高模格式', h=20, fn ='boldLabelFont')
		cmds.setParent('..')
		...
		cmds.text(l='', w=5)
		cmds.checkBox('HideAferMatchCheckBox',l='u匹配后隐藏',v=1, w=90)
		cmds.setParent('..')

		cmds.separator( height=5, style='none' )

		cmds.rowColumnLayout(numberOfColumns=1, adj=1)
		cmds.button(l=u'同时选中高低模进行重命名匹配', h=40, c=self.MatchHighLowPoly)
		cmds.setParent('..')
		...
		#设置快捷键
		cmds.columnLayout()
		cmds.text(l=u' 为该功能设置快捷键', h=20, fn ='boldLabelFont')
		cmds.setParent('..')
		...
		cmds.text(l='', w=5)
		cmds.button(l=u'设置快捷键', w=90, c=self.SetMatchHotkey)
		cmds.setParent('..')
		...
		cmds.text(l='', w=5)
		cmds.checkBoxGrp( 'VisHideLowHigh',numberOfCheckBoxes=2, enable=0,labelArray2=[u'显示低模', u'显示高模'],valueArray2=[1, 1],cc1=self.ShowHideLow,cc2=self.ShowHideHigh)
		cmds.text(l='')
		cmds.button('ShowAll', l=u'全部显示', w=80, enable=1, c=self.ShowAll)
		cmds.setParent('..')
		...
		#检查高低模匹配
		cmds.columnLayout()
		cmds.text(l=u' 检查高低模匹配', h=20, fn ='boldLabelFont')
		cmds.setParent('..')

		cmds.rowLayout(numberOfColumns=1, adj=1)
		cmds.button('CheckMatchButton', enable=1, l=u'检查匹配情况',c=self.CheckListMatch)
		cmds.setParent('..')
		...
		cmds.text('CheckSliderText',l=u' 滑动查看匹配',enable=0)
		...
		cmds.button('ChooseLowPoly', enable=1, l=u'选择低模', w=75, c=self.SelectLowPoly)		
		cmds.button('ChooseHighPoly', enable=1, l=u'选择高模', w=75, c=self.SelectHighPoly)
		cmds.button('ChooseBothPoly', enable=1, l=u'全选', c=self.SelectHighLowPoly)
		cmds.setParent('..')
